...
def _precision_calibration_diag(
    precision,
    raw_precision,
    tempered_precision,
    min_value,
    ...
    small_eps = max(float(eps), 1.0e-12)
    return {
        "raw_precision_mean": round(float(raw_precision.mean().item()), 6),
        "raw_precision_min": round(float(raw_precision.min().item()), 6),
        "raw_precision_max": round(float(raw_precision.max().item()), 6),
        "tempered_precision_mean": round(float(tempered_precision.mean().item()), 6),
        "calibrated_precision_mean": round(float(precision.mean().item()), 6),
        "calibrated_precision_min": round(float(precision.min().item()), 6),
        "calibrated_precision_max": round(float(precision.max().item()), 6),
        "calibrated_precision_std": round(float(precision.std(unbiased=False).item()), 6),
        "precision_at_min_pct": round(
            float((precision <= float(min_value) + small_eps).float().mean().item()),
            6,
        ),
        "precision_at_max_pct": round(
            float((precision >= float(max_value) - small_eps).float().mean().item()),
            6,
        ),
    }
...
def _calibrate_precision_vector(
    ...
    diag = {
        "bayes_precision_mode": mode,
        "bayes_precision_temperature": float(temperature),
        "bayes_precision_target": float(target),
        "bayes_precision_min": float(min_value),
        "bayes_precision_max": float(max_value),
        "param_scale2": None,
        "relative_var_mean": None,
        "relative_var_min": None,
        "relative_var_max": None,
        "raw_precision_mean": None,
        "raw_precision_min": None,
        "raw_precision_max": None,
        "tempered_precision_mean": None,
        "calibrated_precision_mean": None,
        "calibrated_precision_min": None,
        "calibrated_precision_max": None,
        "calibrated_precision_std": None,
        "precision_at_min_pct": None,
        "precision_at_max_pct": None,
    }
    if mode == "floor_inverse":
        effective_var_floor = max(float(var_floor), eps)
        precision = 1.0 / raw_var.clamp(min=effective_var_floor)
        precision = precision.clamp(min=1e-4, max=float(ai_max))
        return precision, effective_var_floor, diag
    ...
    if mode == "normalized_power":
        ...
        precision = precision.clamp(min=min_value, max=max_value)
        diag.update(_precision_calibration_diag(
            precision=precision,
            raw_precision=raw_precision,
            tempered_precision=tempered,
            min_value=min_value,
            max_value=max_value,
            eps=eps,
        ))
        return precision, eps, diag
    ...
    if mode == "relative_normalized_power":
        ...
        param_scale2 = mean_vector.detach().pow(2).mean().clamp_min(eps)
        ...
        tempered_mean = tempered.mean().clamp_min(eps)
        precision = (tempered / tempered_mean * target).clamp(
            min=min_value,
            max=max_value,
        )
        diag.update(
            {
                "param_scale2": round(float(param_scale2.item()), 12),
                "relative_var_mean": round(float(relative_var.mean().item()), 12),
                "relative_var_min": round(float(relative_var.min().item()), 12),
                "relative_var_max": round(float(relative_var.max().item()), 12),
            }
        )
        diag.update(_precision_calibration_diag(
            precision=precision,
    ...
    if mode == "scalar_normalized_power":
        ...
        expanded_raw_precision = torch.empty_like(raw_var)
        expanded_tempered = torch.empty_like(raw_var)
        for scalar_precision, tempered_scalar, (_, start, end) in zip(
            scalar_vector,
            tempered,
            segments,
        ):
            expanded_raw_precision[start:end] = scalar_precision
            expanded_tempered[start:end] = tempered_scalar
        diag.update(_precision_calibration_diag(
            precision=precision,
            raw_precision=expanded_raw_precision,
            tempered_precision=expanded_tempered,
            min_value=min_value,
            max_value=max_value,
            eps=eps,
        ))
...
def _emit_sgld_diag_log(sgld_diag, layer_id, expert_id):
    # client.py intentionally stays untouched; emit the new mode diagnostics through
    # the experiment file logger that has already been configured.
    ...
    message = (
        ...
        f"--bayes_precision_mode:{sgld_diag.get('bayes_precision_mode', sgld_diag.get('precision_mode'))} "
        f"--bayes_precision_temperature:{sgld_diag.get('bayes_precision_temperature', sgld_diag.get('precision_temperature'))} "
        f"--bayes_precision_target:{sgld_diag.get('bayes_precision_target', sgld_diag.get('precision_target'))} "
        f"--bayes_precision_min:{sgld_diag.get('bayes_precision_min')} "
        f"--bayes_precision_max:{sgld_diag.get('bayes_precision_max')} "
        f"--bayes_plain_sgld_noise_scale:{sgld_diag.get('bayes_plain_sgld_noise_scale')} "
        f"--bayes_plain_sgld_loss_scale:{sgld_diag.get('bayes_plain_sgld_loss_scale')} "
        f"--bayes_plain_sgld_prior_precision:{sgld_diag.get('bayes_plain_sgld_prior_precision')} "
        f"--sgld_temperature:{sgld_diag.get('sgld_temperature')} "
        f"--collected_samples:{sgld_diag.get('collected_samples')} "
        f"--data_loss_mean:{sgld_diag.get('data_loss_mean')} "
        f"--prior_loss_mean:{sgld_diag.get('prior_loss_mean')} "
        f"--sgld_loss_mean:{sgld_diag.get('sgld_loss_mean')} "
        ...
        f"--map_pullback_norm_mean:{sgld_diag.get('map_pullback_norm_mean')} "
        f"--sample_pairwise_distance_mean:{sgld_diag.get('sample_pairwise_distance_mean')} "
        f"--param_scale2:{sgld_diag.get('param_scale2')} "
        f"--raw_var_mean:{sgld_diag.get('raw_var_mean')} "
        f"--relative_var_mean:{sgld_diag.get('relative_var_mean')} "
        f"--relative_var_min:{sgld_diag.get('relative_var_min')} "
        f"--relative_var_max:{sgld_diag.get('relative_var_max')} "
        f"--raw_precision_mean:{sgld_diag.get('raw_precision_mean')} "
        f"--raw_precision_min:{sgld_diag.get('raw_precision_min')} "
        f"--raw_precision_max:{sgld_diag.get('raw_precision_max')} "
        f"--tempered_precision_mean:{sgld_diag.get('tempered_precision_mean')} "
        f"--calibrated_precision_mean:{sgld_diag.get('calibrated_precision_mean')} "
        f"--calibrated_precision_min:{sgld_diag.get('calibrated_precision_min')} "
        f"--calibrated_precision_max:{sgld_diag.get('calibrated_precision_max')} "
        f"--calibrated_precision_std:{sgld_diag.get('calibrated_precision_std')} "
        f"--precision_at_min_pct:{sgld_diag.get('precision_at_min_pct')} "
        f"--precision_at_max_pct:{sgld_diag.get('precision_at_max_pct')} "
...
def _run_expert_sgld_fit_adam_noise(
    ...
    with torch.no_grad():
        ...
        if moment1 is None:
            ...
            precision_vector, effective_var_floor, precision_diag = _calibrate_precision_vector(
                raw_var=raw_var,
...
def _run_expert_sgld_fit_two_stage_plain(
    ...
    plain_sgld_sample_interval = max(int(plain_sgld_sample_interval), 1)
    ...
    with torch.no_grad():
        ...
        theta_map = torch.nn.utils.parameters_to_vector(
            theta_map_params
        ).detach().cpu()
    ...
    base_noise_scale = math.sqrt(2.0 * plain_sgld_lr * sgld_temperature)
    noise_scale = plain_sgld_noise_scale * base_noise_scale
    ...
    for step_idx in range(plain_sgld_steps):
        ...
        with torch.no_grad():
            ...
            map_pullback_norm = math.sqrt(max(map_pullback_sq, 0.0))
            param_step_delta_norm = math.sqrt(max(step_delta_sq, 0.0))
            ...
            map_pullback_norms.append(map_pullback_norm)
            param_step_delta_norms.append(param_step_delta_norm)
    ...
    with torch.no_grad():
        ...
        sgld_diag = {
            ...
            "bayes_plain_sgld_noise_scale": float(plain_sgld_noise_scale),
            "bayes_plain_sgld_loss_scale": float(plain_sgld_loss_scale),
            "bayes_plain_sgld_prior_precision": float(plain_sgld_prior_precision),
            "bayes_precision_mode": str(precision_mode),
            "bayes_precision_temperature": float(precision_temperature),
            "bayes_precision_target": float(precision_target),
            "bayes_precision_min": float(precision_min),
            "bayes_precision_max": float(precision_max),
            "bayes_precision_eps": float(precision_eps),
            ...
            "sample_variance_mean": None,
            "sample_variance_min": None,
            "sample_variance_max": None,
            ...
            sgld_diag.update(
                {
                    ...
                    "sample_variance_min": 0.0,
                    "sample_variance_max": 0.0,
                    "raw_var_mean": 0.0,
                    "raw_var_min": 0.0,
                    "raw_var_max": 0.0,
            ...
            sgld_diag.update(
                {
                    **precision_diag,
                    ...
                    "sample_variance_mean": round(float(raw_var.mean().item()), 12),
                    "sample_variance_min": round(float(raw_var.min().item()), 12),
                    "sample_variance_max": round(float(raw_var.max().item()), 12),
                    "raw_var_mean": round(float(raw_var.mean().item()), 12),
                    "raw_var_min": round(float(raw_var.min().item()), 12),
                    "raw_var_max": round(float(raw_var.max().item()), 12),
                    "raw_var_under_floor_pct": round(
        ...
        sgld_diag.update(
            {
                ...
                "plain_sgld_burnin": None,
                "plain_sgld_lr": None,
                "bayes_plain_sgld_noise_scale": None,
                ...
                "map_param_delta_rel": None,
                "final_param_delta_rel": None,
                "mean_state_delta_rel": None,
                "sample_pairwise_distance_mean": None,
                "sample_pairwise_distance_max": None,
                "sample_variance_mean": sgld_diag.get("raw_var_mean"),
                "sample_variance_min": sgld_diag.get("raw_var_min"),
                "sample_variance_max": sgld_diag.get("raw_var_max"),
                "grad_update_norm_mean": None,
                "noise_update_norm_mean": None,
                "noise_to_grad_ratio_mean": None,
                "param_step_delta_norm_mean": None,
            }
        )
        return mean_state, precision_state, sgld_diag
...
def configure_bayes_sgld_fit_defaults(config):
    """Apply YAML-level defaults without requiring a client.py call-site change."""
